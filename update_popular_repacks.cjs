...
const targetComp = `const MostPopularRepacksSection: React.FC<{ 
    gameIds: string[], 
...
const newComp = `const MostPopularRepacksModal: React.FC<{
    games: ResourceItem[],
    onClose: () => void,
    onSelect: (item: ResourceItem) => void
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-900/80 backdrop-blur-md">
            <motion.div 
                ...
                className="bg-white dark:bg-slate-900 w-full max-w-7xl max-h-full rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800"
            >
                            ...
                            return (
                                ...
                                >
                                    <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-slate-200 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-md transition-transform duration-300 group-hover:scale-105 group-hover:shadow-xl group-hover:shadow-blue-500/20">
                                        ...
                                        {isHypervisor && (
                                            <div className="absolute top-2 right-2 bg-red-600/90 backdrop-blur-md text-white font-black text-[10px] sm:text-xs px-2 py-1 rounded-lg shadow-lg border border-red-400/30">
                                                HV
                                            </div>
                                        )}
            ...
            {games.length > displayCount && (
                <div className="mt-8 flex justify-center">
                    <button 
                        ...
                        className="px-6 py-3 bg-blue-600/10 hover:bg-blue-600/20 text-blue-600 dark:text-blue-400 dark:bg-blue-500/10 dark:hover:bg-blue-500/20 font-bold uppercase tracking-wider rounded-xl transition-all border border-blue-600/20 dark:border-blue-500/30 flex items-center gap-2"
                    >
...
if (code.includes(targetComp)) {
    ...
    console.log("Replaced MostPopularRepacksSection");
}
    const startIndex = code.indexOf('const MostPopularRepacksSection: React.FC');
    const endIndex = code.indexOf('};', startIndex) + 2; // finding the end of the functional component
    // we have multiple }; so we must be careful or just use replace.
    // The previous component ends with:
    //         </div>
    //     </div>
    //     );
    // };
    // Let's replace the whole string we matched earlier
}
fs.writeFileSync('update_popular_repacks.cjs', fs.readFileSync('update_popular_repacks.cjs', 'utf8').replace('if (code.includes(\'MostPopularRepacksSection: React.FC\')) {', `if (code.includes(targetComp)) {
    code = code.replace(targetComp, newComp);
    fs.writeFileSync('pages/SecretArea.tsx', code);
    console.log("Replaced MostPopularRepacksSection");
}`));
