...
// Fix Icon.tsx
let iconContent = fs.readFileSync('components/Icon.tsx', 'utf8');

if (!iconContent.includes('TbShare')) {
    iconContent = iconContent.replace(
        /import \{([\s\S]*?)\} from 'react-icons\/tb';/,
        "import {$1, TbShare} from 'react-icons/tb';"
    );
}
if (!iconContent.includes('Share2: TbShare')) {
    iconContent = iconContent.replace(
...
fs.writeFileSync('components/Icon.tsx', iconContent);
console.log("components/Icon.tsx updated");
